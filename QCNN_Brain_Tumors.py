...
output_dir = r'C:\Users\Michelle Wu\OneDrive\Desktop\UCR\MATH\MATH194\Code Outputs'

if not os.path.exists(data_dir):
    print(f"The directory {data_dir} does not exist. Please provide the correct path.")
    sys.exit()
...
total_images_loaded = sum([len(files) for r, d, files in os.walk(data_dir) if files])
...
X_reduced_mmap = np.memmap('X_reduced.dat', dtype=np.float32, mode='w+', shape=(total_images_loaded, num_components))
y_mmap = np.memmap('y.dat', dtype=np.float32, mode='w+', shape=(total_images_loaded, 4))
...
for batch_index in range(batch_index):
    ...
    try:
        X_reduced_mmap[index:index + current_batch_size, :X_reduced_batch.shape[1]] = X_reduced_batch
...
X_reduced_mmap = np.memmap('X_reduced.dat', dtype=np.float32, mode='r', shape=(total_images_loaded, num_components))
y_mmap = np.memmap('y.dat', dtype=np.float32, mode='r', shape=(total_images_loaded, 4))
...
X_train, X_test, y_train, y_test = train_test_split(X_reduced, y_reduced_1d, test_size=0.2, random_state=42)
...
class QuantumLayer(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        print(f"QuantumLayer build with input shape {input_shape} and weights shape {self.q_weights.shape}")
    ...
    def call(self, inputs):
        ...
        quantum_output = tf.map_fn(lambda x: tf.py_function(quantum_circuit, [x, self.q_weights], tf.float32), inputs)
...
# Compile the model
qcnn_model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
...
print(f"X_train_expanded shape: {X_train_expanded.shape}, X_test_expanded shape: {X_test_expanded.shape}")
...
history = qcnn_model.fit(X_train_expanded, y_train_one_hot, epochs=100, batch_size=32, validation_split=0.2, callbacks=[early_stopping])
...
for i in range(len(svd_transformers)):
    for chunk_index in range(0, 64, chunk_size):
        ...
        try:
            ...
            X_reconstructed_chunk = X_reconstructed_chunk.reshape(-1, 240, 240, 4)  # Example shape, update as needed
